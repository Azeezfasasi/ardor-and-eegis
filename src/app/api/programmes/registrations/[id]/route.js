...
export async function PUT(req, { params }) {
  try {
    ...
    // Update Programme registered count based on status change
    const programme = await Programme.findOne({ programmeName: registration.programmeName });

    if (programme) {
      // If transitioning to cancelled, decrement count
      if (oldStatus !== "cancelled" && registrationStatus === "cancelled") {
        programme.registeredCount = Math.max(0, (programme.registeredCount || 0) - 1);
      }
      // If transitioning from cancelled to active, increment count
      else if (oldStatus === "cancelled" && registrationStatus !== "cancelled") {
        programme.registeredCount = (programme.registeredCount || 0) + 1;
      }
