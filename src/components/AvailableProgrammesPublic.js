...
export default function AvailableProgrammesPublic() {
  ...
  const fetchProgrammes = async () => {
    ...
    try {
      ...
      // Uncomment the next line to debug API responses in development only
      // if (process.env.NODE_ENV === 'development') console.log('Programmes API Response:', data);
      ...
      // Extract programmes from API response
      const programmesArray = data.programmes || [];

      if (process.env.NODE_ENV === 'development') {
        console.log('Programmes fetched:', programmesArray.length);
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 mt-[90px]">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-purple-700 via-purple-600 to-purple-700 text-white py-12 md:py-20 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-3xl md:text-5xl font-bold mb-4">Discover Our Programmes</h1>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="w-full bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{ width: `${programme.capacity > 0 ? Math.min((programme.registeredCount || 0) / programme.capacity * 100, 100) : 0}%` }}
                                ></div>
                              </div>
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 animate-in fade-in zoom-in">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => router.push(`/id-finder?programmeId=${encodeURIComponent(enrollModal._id)}`)}
