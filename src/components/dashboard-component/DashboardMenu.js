...
export default function DashboardMenu({ collapsed, mobileOpen = false, onClose = () => { } }) {
  ...
  const items = [
    ...
    { href: '/dashboard/quote-requests', label: 'Quote Requests', icon: 'dashboard', roles: ['admin'] },
    { href: '/dashboard/contact-form-responses', label: 'Contact Form Responses', icon: 'dashboard', roles: ['admin'] },
    {
      href: '/dashboard/our-blogs',
      label: 'Blog Management',
      icon: 'Users',
      roles: ['admin'],
      children: [
        { href: '/dashboard/manage-blog', label: 'All Blog Posts', roles: ['admin'] },
        { href: '/dashboard/add-blog', label: 'Add New', roles: ['admin'] },
      ]
    },
    {
      ...
      roles: ['admin'],
      children: [
        { href: '/dashboard/all-gallery', label: 'All Gallery', roles: ['admin'] },
    ...
    { href: '/dashboard/manage-career-jobs', label: 'Manage Career Jobs', icon: 'projects', roles: ['admin', 'staff-member'] },
    {
      ...
      roles: ['admin'],
      children: [
        { href: '/dashboard/manage-message-slider', label: 'Message Slider', roles: ['admin'] },
        { href: '/dashboard/hero-slider', label: 'Hero Slider', roles: ['admin'] },
        { href: '/dashboard/why-choose-us', label: 'Why Choose Us', roles: ['admin'] },
        { href: '/dashboard/our-services-contents', label: 'Our Services', roles: ['admin'] },
        { href: '/dashboard/home-cta', label: 'About Section', roles: ['admin'] },
  ...
  const desktopNav = (
    <nav className={`hidden md:flex h-full bg-[#7B542F] border-r border-gray-100 ${collapsed ? 'w-16' : 'w-75'} transition-width duration-200`} aria-label="Dashboard navigation">
                ...
                {hasChildren ? (
                  <div>
                    <button
                      ...
                      className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-md ${active ? 'bg-indigo-50 text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}
                    >
                    ...
                    {!collapsed && isOpen && (
                      <ul className="mt-1 space-y-1 pl-10">
                        {i.children.map(c => (
                          <li key={c.href}>
                            <Link href={c.href} className={`block px-3 py-2 rounded-md text-sm ${pathname === c.href ? 'bg-indigo-50 text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}>
                              {c.label}
                            </Link>
                          </li>
                        ))}
                  ...
                  <Link href={i.href} className={`flex items-center gap-3 px-3 py-2 rounded-md ${active ? 'bg-indigo-50 text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}>
                    <span className="shrink-0"> <Icon name={i.icon} /> </span>
  ...
  const mobileNav = mobileOpen ? (
    <div className="fixed inset-0 z-40 md:hidden" role="dialog" aria-modal="true">
      ...
      <nav className="relative z-50 h-full w-[80%] bg-[#7B542F] border-r border-gray-100">
        <div className="h-full overflow-y-auto py-6 px-4">
          <div className="flex items-center justify-between mb-6">
            <Link href="/" className="flex flex-col items-center gap-3">
              <Image src="/img/ardorlogotrans.png" alt="Ardor Aegis Logo" width={170} height={50} className="w-20 block rounded-md" />
            </Link>
              ...
              {items.map(i => {
                ...
                return (
                  <li key={i.href}>
                    {hasChildren ? (
                      <div>
                        <button onClick={() => toggleSub(i.href)} className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-md ${active ? 'bg-white text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}>
                        ...
                        {isOpen && (
                          <ul className="mt-1 space-y-1 pl-6">
                            {i.children.map(c => (
                              <li key={c.href}>
                                <Link href={c.href} onClick={onClose} className={`block px-3 py-2 rounded-md text-sm ${pathname === c.href ? 'bg-indigo-50 text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}>
                                  {c.label}
                                </Link>
                              </li>
                            ))}
                      ...
                      <Link href={i.href} onClick={onClose} className={`flex items-center gap-3 px-3 py-2 rounded-md ${active ? 'bg-purple-50 text-[#7B542F]' : 'text-white hover:bg-[#7B542F]/80'}`}>
                        <span className="shrink-0 text-white"> <Icon name={i.icon} /> </span>
                        <span className="text-sm font-medium">{i.label}</span>
                      </Link>
                    )}
